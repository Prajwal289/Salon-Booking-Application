const login = '/user/login';
const registerRoute = '/user/register';
const allServices = '/user/services';
const offers = '/user/offers';
const allAppointments = '/employee/appointments';
const addService = '/admin/addservice';
const addOffer = '/admin/addoffer';
const addBeautician = '/admin/addbeautician';
const bookAppointment = '/customer/bookapppointment';
const userAppointment = '/customer/userappointments';
const cancelAppointment = '/customer/cancelappointments';

export {
    login, registerRoute, allServices, allAppointments, addService, bookAppointment, addOffer, addBeautician,offers, userAppointment, cancelAppointment
}